...
// Anchored: validates a single, isolated email field (structured CV data).
...
function scoreWork(data: CvData): SectionReport {
  ...
  if (missingDesc > 0) critical.push("Missing or unclear role descriptions");
  if (withMetric < items.length) critical.push("Lack of measurable impact or results");
  if (withVerb < items.length) suggestions.push("Start bullets with strong action verbs");
  ...
  if (withMetric > 0) goodPractices.push("Some achievements are quantified with metrics");
...
function scoreSimpleSection(
  ...
  const score = total / items.length;
  if (score >= 80) goodPractices.push(`${SECTION_LABELS[key]} entries are well described`);
...
export function scoreCv(data: CvData): ScoreReport {
  ...
  const coreAvg = core.reduce((sum, s) => sum + s.score, 0) / (core.length || 1);
  const optionalFilled = optional.filter((s) => s.score > 0);
  const optionalBonus =
    optionalFilled.length > 0
      ? (optionalFilled.reduce((sum, s) => sum + s.score, 0) / optionalFilled.length) * 0.12
      : 0;
...
export function scoreRawText(text: string): ScoreReport {
  ...
  const hasEmail = EMAIL_SCAN_RE.test(text);
  ...
  const generalScore = clamp(contactScore * 0.3 + lengthScore * 0.5 + metricBonus);
